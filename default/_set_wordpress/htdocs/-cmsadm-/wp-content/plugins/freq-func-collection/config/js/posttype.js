;jQuery(function ($) {
    ...
    ['posttype', 'metabox', 'metaboxField', 'taxonomy', 'taxonomyField'].forEach(function (type) {
    ...
    // アイテム追加
    $('body').on('click', '.addPosttypeButton', function (e) {
        var $element = $($('#templatePosttypeItem')
            .html()
            .replace(/\{\{POSTTYPE_NO\}\}/g, getNextNo($(this), 'posttype')));
        sortableList($element.find('.metaboxList'), 'metabox');
        sortableList($element.find('.taxonomyList'), 'taxonomy');
        $element.appendTo($(this).prev('.posttypeList'));
    });
    $('body').on('click', '.addMetaboxButton', function (e) {
        var $element = $($('#templateMetaboxItem')
            .html()
            .replace(/\{\{POSTTYPE_NO\}\}/g, getParentNo($(this), 'posttype'))
            .replace(/\{\{METABOX_NO\}\}/g, getNextNo($(this), 'metabox')));
        sortableList($element.find('.metaboxFieldList'), 'metaboxField');
        $element.appendTo($(this).prev('.metaboxList'));
    });
    $('body').on('click', '.addTaxonomyButton', function (e) {
        var $element = $($('#templateTaxonomyItem')
            .html()
            .replace(/\{\{POSTTYPE_NO\}\}/g, getParentNo($(this), 'posttype'))
            .replace(/\{\{TAXONOMY_NO\}\}/g, getNextNo($(this), 'taxonomy')));
        sortableList($element.find('.taxonomyFieldList'), 'taxonomyField');
        $element.appendTo($(this).prev('.taxonomyList'));
    });
    $('body').on('click', '.addMetaboxFieldButton', function (e) {
        var $element = $($('#templateMetaboxFieldItem')
            .html()
            .replace(/\{\{POSTTYPE_NO\}\}/g, getParentNo($(this), 'posttype'))
            .replace(/\{\{METABOX_NO\}\}/g, getParentNo($(this), 'metabox'))
            .replace(/\{\{METABOX_FIELD_NO\}\}/g, getNextNo($(this), 'metaboxField')));
        $element.appendTo($(this).prev('.metaboxFieldList'));
    });
    $('body').on('click', '.addTaxonomyFieldButton', function (e) {
        var $element = $($('#templateTaxonomyFieldItem')
            .html()
            .replace(/\{\{POSTTYPE_NO\}\}/g, getParentNo($(this), 'posttype'))
            .replace(/\{\{TAXONOMY_NO\}\}/g, getParentNo($(this), 'taxonomy'))
            .replace(/\{\{TAXONOMY_FIELD_NO\}\}/g, getNextNo($(this), 'taxonomyField')));
        $element.appendTo($(this).prev('.taxonomyFieldList'));
    });
    ...
    $('body').on('click', '.viewExampleCode', function (e) {
        ...
        var html = exampleCode[dir][type].replace(/\{\{KEY\}\}/g, key);
