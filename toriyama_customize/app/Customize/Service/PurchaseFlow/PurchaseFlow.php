...
use Eccube\Service\PurchaseFlow\PurchaseFlow as EccubePurchaseFlow;
...
class PurchaseFlow extends EccubePurchaseFlow
{
    /**
     * @var string
     */
     ...
     * @param ItemHolderInterface $itemHolder
     */
    protected function calculateDeliveryFeeDiscountTotal(ItemHolderInterface $itemHolder)
    {
        $total = $itemHolder->getCustomItems()
            ->getDiscountDeliveryFees()
            ->reduce(function ($sum, ItemInterface $item) {
                $sum += $item->getPriceIncTax() * $item->getQuantity();

                return $sum;
            }, 0);
        $itemHolder->setDeliveryFeeDiscountTotal($total);
    }
    ...
    protected function calculateAll(ItemHolderInterface $itemHolder)
    {
        ...
        $this->calculateDeliveryFeeDiscountTotal($itemHolder);
